...
curl -i -H "Content-Type: application/json" -X POST -d '{"swaggerURL":"http://169.44.118.61:8080/TomcatWebAppForChefNode/swagger.json"}' http://169.44.118.61:8080/SwaggerParser/api/swaggerTestService | sed 's/\r//' > curl.out
printf "\n" >> curl.out
